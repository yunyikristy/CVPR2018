...
# reduce_mean normalize also the dimension of the conditions
...
class CondGANTrainer(object):
    ...
    # def sample_encoded_context(self, conditions):
    #     '''Helper function for init_opt'''
    #     c_mean_logsigma = self.model.generate_condition(conditions)
    ...
    def init_opt(self):
        ...
        with tf.variable_scope("g_net"):
            with pt.defaults_scope(phase=pt.Phase.train):
                ...
                # print('#########################')
                G_input = self.model.generate_condition(tf.concat(1, [c_text, z]), c_mask)
                # update by zhaoyang
    ...
    def sampler(self):
        # c, _ = self.sample_encoded_context(self.conditions)
        ...
        z = tf.random_normal([self.batch_size, cfg.Z_DIM])
        G_input = self.model.generate_condition(tf.concat(1, [c_text, z]), c_mask)
        self.fake_images = self.model.get_generator(G_input)

    def compute_losses(self, images, wrong_images, fake_images, embeddings, conditions):
        real_logit = self.model.get_discriminator(images, embeddings, conditions)
        wrong_logit = self.model.get_discriminator(wrong_images, embeddings, conditions)
        fake_logit = self.model.get_discriminator(fake_images, embeddings, conditions)
    ...
    def visualization(self, n):
        fake_sum_train, superimage_train = \
            self.visualize_one_superimage(self.debug_fake_images[:n * n],
                                          self.images[:n * n],
                                          n, "train")
        fake_sum_test, superimage_test = \
            self.visualize_one_superimage(self.debug_fake_images[n * n:2 * n * n],
    ...
    def preprocess(self, x, n):
        # make sure every row with n column have the same conditions
        for i in range(n):
            for j in range(1, n):
                x[i * n + j] = x[i * n]
        return x

    def epoch_sum_images(self, sess, n, counter):
        images_train, _, conditions_train, embeddings_train, captions_train, _ =\
            self.dataset.train.next_batch(n * n, cfg.TRAIN.NUM_EMBEDDING)
        images_train = self.preprocess(images_train, n)
        conditions_train = self.preprocess(conditions_train, n)
        embeddings_train = self.preprocess(embeddings_train, n)

        images_test, _, conditions_test, embeddings_test, captions_test, _ = \
            self.dataset.test.next_batch(n * n, 1)
        images_test = self.preprocess(images_test, n)
        conditions_test = self.preprocess(conditions_test, n)
        embeddings_test = self.preprocess(embeddings_test, n)

        images = np.concatenate([images_train, images_test], axis=0)
        conditions =\
            np.concatenate([conditions_train, conditions_test], axis=0)
        embeddings =\
            np.concatenate([embeddings_train, embeddings_test], axis=0)

        if self.batch_size > 2 * n * n:
            images_pad, _, conditions_pad, embeddings_pad, _, _ =\
                self.dataset.test.next_batch(self.batch_size - 2 * n * n, 1)
            
            #print ('embedding and embedding pad shape is:', embeddings.shape, embeddings_pad.shape)
            images = np.concatenate([images, images_pad], axis=0)
            conditions = np.concatenate([conditions, conditions_pad], axis=0)
            embeddings = np.concatenate([embeddings, embeddings_pad], axis=0)
        feed_dict = {self.images: images,
                     self.conditions: conditions,
                     self.embeddings: embeddings}
        fake, gen_samples, img_summary =\
            sess.run([self.debug_fake_images[0], self.superimages, self.image_summary], feed_dict)
        #print('fake image is:', fake)
        #print('gen_sampls[0]', gen_samples[0])
        # save images generated for train and test captions
        
        #scipy.misc.imsave('%s/fake.jpg' % (self.log_dir), fake)
        scipy.misc.imsave('%s/train_%d.jpg' % (self.log_dir, counter), gen_samples[0])
        scipy.misc.imsave('%s/test_%d.jpg' % (self.log_dir, counter), gen_samples[1])
        
        ########## added by kristy -- visualize the seg condition#######################
        # print ('conditions shape is', conditions.shape)
        # scipy.misc.imsave('%s/condition_test.jpg' % (self.log_dir), conditions[0])
    

        # pfi_train = open(self.log_dir + "/train.txt", "w")
        pfi_test = open(self.log_dir + "/test_%d.txt" %counter, "w")
        for row in range(n):
            # pfi_train.write('\n***row %d***\n' % row)
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(epoch_start, self.max_epoch):
                ...
                for i in range(updates_per_epoch):
                    pbar.update(i)
                    # training d
                    images, wrong_images, conditions, embeddings, _, _ =\
                        self.dataset.train.next_batch(self.batch_size, num_embedding)

                    # print('##########saving images and conditions ...######')
                    # scipy.misc.imsave('image_%d.jpg' %counter, images[0])
                    # scipy.misc.imsave('wrong_images_%d.jpg' %counter, wrong_images[0])
                    # scipy.misc.imsave('condition_%d.jpg' %counter, conditions[0])
                    # print('##############complete!########################')
                    ...
                    _, d_sum, hist_sum, log_vals = sess.run(feed_out,
                                                            feed_dict)
                    # feed_out = [self.discriminator_trainer, log_vars]
                    # _, log_vals = sess.run(feed_out,feed_dict)
                    ...
                    # # update by zhaoyang###############################
                    # # this part of code is very very very important
                    # # feed_out = [self.discriminator_trainer, log_vars, self.debug_g_input]
                    # # _, log_vals, debug_output = sess.run(feed_out,feed_dict)
                    #feed_out = [self.debug_g_input, self.fake_images]
                    #debug_output, debug_img = sess.run(feed_out,feed_dict)

                    #fake_pic = np.empty(debug_img[0].shape)
                    #fake_pic = debug_img[0]
                    
                    #import scipy.misc
                    #print('debug info:########')
                    # #print('G_input is :', debug_output)
                    # #print('fake_image is:', debug_img)

                    #img_name = 'fake_img_%d.jpg' %(counter)
                    #scipy.misc.imsave(img_name,fake_pic)
                ...
                img_sum = self.epoch_sum_images(sess, cfg.TRAIN.NUM_COPY, counter)
    ...
    def eval_one_dataset(self, sess, dataset, save_dir, subset='train'):
        ...
        while count < dataset._num_examples:
            ...
            images, conditions_batchs, embeddings_batches,
            filenames, _ =\
                dataset.next_batch_test(self.batch_size, start, 1)
            print('count = ', count, 'start = ', start)
            for i in range(len(conditions_batchs)):
                samples_batchs = []
                ...
                for j in range(np.minimum(16, cfg.TRAIN.NUM_COPY)):
                    samples = sess.run(self.fake_images,
                                       {self.conditions: conditions_batchs[i]})
