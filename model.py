...
class CondGAN(object):
    ...
    # g-net
    '''
    def generate_text_condition(self, c_var):
        conditions =\
            (pt.wrap(c_var).
             flatten().
    ...
    def generate_condition(self, c_var):
        ...
        condition_vectors =\
            (pt.wrap(c_var).
             ...
             apply(leaky_rectify, leakiness=0.2))
        print ("conditions shape is:" )
        print (conditions.shape)
        print (condition_vectors.shape)
        # mean = conditions[:, :self.ef_dim]
        # log_sigma = conditions[:, self.ef_dim:]
        # return [mean, log_sigma]
        return [conditions, condition_vectors]
    '''
    def generate_condition(self, c_var, c_mask):
        condition_vec =\
                ...
                conv_batch_norm().
                apply(tf.nn.relu))
        # print('condition_vec shape is:', condition_vec.shape)
        ...
        condition_mask =\
                ...
                conv_batch_norm().
                apply(tf.nn.relu))
        # print('condition_mask shape is:', condition_mask.shape)

        condition = tf.concat(3, [condition_vec, condition_mask])
        # print('concatented condition shape is:', condition)
        return condition
    ...
    def get_discriminator(self, x_var, c_text, c_var):
        ...
        # print('x_code shape is:', x_code.shape)

        c_code = self.d_encode_img_template.construct(input=c_var)
        # print('c_code shape is:', c_code.shape)
